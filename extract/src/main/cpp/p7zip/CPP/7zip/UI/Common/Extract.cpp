...
static HRESULT DecompressArchive(
    ...
    CExtractCallbackConsole *extractCallbackConsole)
{
  ...
  if (!options.StdInMode)
  {
    ...
    {
      extractCallbackConsole->ThereAreNoFiles();
      return extractCallbackConsole->ExtractResult(S_OK);
    }
  }
  ...
  */
  ...
  return extractCallbackConsole->ExtractResult(result);
}
    ...
    CExtractCallbackConsole *extractCallbackConsole)
{
  ...
  if (multi)
  {
    RINOK(extractCallbackConsole->SetTotal(totalPackSize));
  }
  ...
  for (i = 0; i < numArcs; i++)
  {
    ...
    */
    ...
    RINOK(extractCallbackConsole->BeforeOpen(arcPath, options.TestMode));
    ...
    */
    ...
    RINOK(extractCallbackConsole->OpenResult(codecs, arcLink, arcPath, result));
    ...
    /*
    // Now openCallback and extractCallbackConsole use same object. So we don't need to send password.
    ...
    {
      RINOK(extractCallbackConsole->SetPassword(password));
    }
    #endif
    */
    ...
    RINOK(DecompressArchive(
        ...
        options,
        calcCrc,
        extractCallback, ecs, errorMessage, packProcessed, extractCallbackConsole));
  ...
  {
    RINOK(extractCallbackConsole->SetTotal(totalPackSize));
    RINOK(extractCallbackConsole->SetCompleted(&totalPackProcessed));
  }
