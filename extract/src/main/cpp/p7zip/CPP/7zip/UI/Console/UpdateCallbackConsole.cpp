...
extern jmethodID updateSetTotal, updateSetRatioInfo;
extern jmethodID updateSetCompleted;
...
HRESULT CUpdateCallbackConsole::ScanProgress(const CDirItemsStat &st, const FString &path,
                                             bool /* isDir */) {
    if (NeedPercents()) {
...
//  }
    ...
    if (enviro.env) {
        if (enviro.obj != nullptr) {
            enviro.env->CallLongMethod(enviro.obj, updateSetTotal, (jlong) size);
        }
    }
    return S_OK;
}
...
HRESULT CUpdateCallbackConsole::SetCompleted(const UInt64 *completeValue) {
    MT_LOCK
    if (completeValue != nullptr && updateSetCompleted) {
        LOGE("AAAAA completed %u", *completeValue);
//    if (NeedPercents()) {
...
HRESULT CUpdateCallbackConsole::SetRatioInfo(const UInt64 *inSize, const UInt64 *outSize) {
    if (inSize && outSize)
        if (enviro.env && enviro.obj != nullptr) {
...
HRESULT
CCallbackConsoleBase::PrintProgress(const wchar_t *name, const char *command, bool showInLog) {
    MT_LOCK
...
HRESULT
CUpdateCallbackConsole::GetStream(const wchar_t *name, bool /* isDir */, bool isAnti, UInt32 mode) {
    if (StdOutMode)
        return S_OK;
...
HRESULT
CUpdateCallbackConsole::ReportExtractResult(Int32 opRes, Int32 isEncrypted, const wchar_t *name) {
    LOGE("AAA ===> ReportExtractResult");
...
HRESULT
CUpdateCallbackConsole::ReportUpdateOpeartion(UInt32 op, const wchar_t *name, bool /* isDir */) {
    // if (StdOutMode) return S_OK;
    ...
    switch (op) {
        case NUpdateNotifyOp::kAdd:
            s = "+";
            break;
        case NUpdateNotifyOp::kUpdate:
            s = "U";
            break;
        case NUpdateNotifyOp::kAnalyze:
            s = "A";
            requiredLevel = 3;
            break;
        case NUpdateNotifyOp::kReplicate:
            s = "=";
            requiredLevel = 3;
            break;
        case NUpdateNotifyOp::kRepack:
            s = "R";
            requiredLevel = 2;
            break;
        case NUpdateNotifyOp::kSkip:
            s = ".";
            requiredLevel = 2;
            break;
        case NUpdateNotifyOp::kDelete:
            s = "D";
            requiredLevel = 3;
            break;
        case NUpdateNotifyOp::kHeader:
            s = "Header creation";
            requiredLevel = 100;
            break;
        default: {
